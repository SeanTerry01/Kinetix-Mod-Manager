...
| `KinetixModManager.Tests` | `net10.0` | 1,327 xUnit tests against `Kinetix.Core` and `Kinetix.Platform.Linux`. |
| `Kinetix.Platform.Linux` | `net10.0` | The Linux answers to the same platform questions: speech through speech-dispatcher, sounds through GStreamer, secrets through libsecret, and finding an installed game through Steam (four layouts, Flatpak included) and its Proton prefixes. No UI toolkit. |
| `Kinetix.Gtk` | `net10.0` | A GTK4 front end for Linux: games, installed mods, mod search and install, updates, profiles, an embedded wiki Orca reads, per-game sounds and settings. Eight tabs against the Windows head's sixty-odd; Nexus-sourced games cannot be searched from here yet and say so. See `ARCHITECTURE_REVIEW.md` §17 and §28–§30. |
