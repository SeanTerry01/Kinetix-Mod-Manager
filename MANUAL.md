...
2.  **Types**: Use the **Type** dropdown to choose what the tab lists. Each one asks Nexus a different question, and the order results come back in is what really separates them:
    ...
    *   **Nexus Categories** browses one category of the game's catalogue, **most downloaded first** — see the next step.
    Every mode except **Search** ignores the search box.
3.  **Nexus Category Filter**: Use the **Nexus category** dropdown to restrict results to one of Nexus's own categories — Armour, Gameplay, Audio, Patches, and so on. The list is built from the categories that **actually have mods for the game you're managing**, most-populated first and with a count beside each, so you see "Armour (1247)" rather than a long list of categories this game has never used. Choose **"Any category"** at the top to turn the filter off.
    *   **It narrows whatever the Type dropdown is doing.** Type *dragon* with **Search** and pick **Armour**, and you get dragon armour mods; leave the box empty, choose **Most Popular** and pick **Audio**, and you get the game's best-known sound mods. It works the same way with All, Trending and Recent.
    *   **Or use it on its own.** Set **Type** to **Nexus Categories**, pick a category, and press **Search**: you get that category's mods with the most downloaded first, which is what you usually want when browsing a category you don't know yet. If you choose that Type without picking a category, the manager says so and moves you to the category list rather than quietly listing something else.
    *   Unlike the Language filter, your choice here is **not remembered between sessions** and resets to "Any category" — a category is a choice about the search in front of you rather than a standing preference. It also reloads when you switch games, since Skyrim's categories are nothing like Stardew Valley's.
...
5.  **What each result tells you**: A result reads as its **name and Nexus ID**, then **"Installed"** if you already have it, then **how many downloads and endorsements** it has, then its **short description** — for example *"Serena's Grimoire (ID: 23). Installed. 3,428 downloads, 50 endorsements. Dark magic, dramatic rituals…"*. Everything before the description is there so you can move on to the next result without sitting through a description you've already ruled out.
    ...
    *   The check runs **when the results are fetched**. If you install something from the results list, that row keeps its original wording until you search again.
